...
When triggered by webhook, this script makes slices from videos in your `/data/src-video/` path and arranges them in one or more compilation videos saved to your `/data/dest-video/` path, according to the instructions you give it using CSV placed in your `/data/manifest/` path.
